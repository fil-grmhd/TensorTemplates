...
*/
    ...
    tensor_field_t<vector3_t<CCTK_REAL>> vel(velx,vely,velz);
    ...
    tensor_field_t<metric_tensor_t<CCTK_REAL,3>> metric(gxx,gxy,gxz,
                                                     gxy,gyy,gyz,
                                                     gxz,gyz,gzz);
...
#pragma omp parallel
    {
        UTILS_LOOP3(thc_grsource_templates,
            ...
            invmetric_tensor_t inv_metric(uxx,uxy,uxz,
                                          uxy,uyy,uyz,
                                          uxz,uyz,uzz);
            ...
            auto velshift = vel[ijk] - beta[ijk]/alp[ijk];
            ...
            auto vel_low = contract<0,0>(metric[ijk],vel[ijk])
            ...
            // this is not optimal, em_tensor should be constructed directly to values below
            // implicitly setting tensor components to zero on construction
            tensor4_t<CCTK_REAL, lower_t, lower_t> em_tensor;
            // this is really ugly, we should implement (i,j,k,...) operator
            em_tensor<0,0>() = (rho_h_W2 - press[ijk])/SQ(alp[ijk]);

            em_tensor<0,1>() = rho_h_W2*velxshift/alp[ijk] +
                               press[ijk]*betax[ijk]/SQ(alp[ijk]);
            em_tensor<0,2>() = rho_h_W2*velyshift/alp[ijk] +
                               press[ijk]*betay[ijk]/SQ(alp[ijk]);
            em_tensor<0,3>() = rho_h_W2*velzshift/alp[ijk] +
                               press[ijk]*betaz[ijk]/SQ(alp[ijk]);

            em_tensor<1,1>() = rho_h_W2*velxshift*velxshift +
                               press[ijk]*(uxx - betax[ijk]*betax[ijk]/SQ(alp[ijk]));
            em_tensor<1,2>() = rho_h_W2*velxshift*velyshift +
                               press[ijk]*(uxy - betax[ijk]*betay[ijk]/SQ(alp[ijk]));
            em_tensor<1,3>() = rho_h_W2*velxshift*velzshift +
                               press[ijk]*(uxz - betax[ijk]*betaz[ijk]/SQ(alp[ijk]));
            em_tensor<2,2>() = rho_h_W2*velyshift*velyshift +
                               press[ijk]*(uyy - betay[ijk]*betay[ijk]/SQ(alp[ijk]));
            em_tensor<2,3>() = rho_h_W2*velyshift*velzshift +
                               press[ijk]*(uyz - betay[ijk]*betaz[ijk]/SQ(alp[ijk]));
            em_tensor<3,3>() = rho_h_W2*velzshift*velzshift +
                               press[ijk]*(uzz - betaz[ijk]*betaz[ijk]/SQ(alp[ijk]));
            // we need to manually symmetrize, we should write at least a member automating this over two indices
