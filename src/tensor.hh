...
template <typename T, typename frame_t_, typename symmetry_t_, size_t rank_,
          typename index_t_, size_t ndim_>
...
public:
  ...
  general_tensor_t(tensor_expression_t<E> const &tensor_expression,
    ...
    //    static_assert(std::is_same<frame_t, typename
    //    E::property_t::frame_t>::value,
    ...
    static_assert(
        compare_index_types<index_t, typename E::property_t::index_t, rank>(),
  ...
  // SYM: this is in weird order for sym2 and rank > 2, first sym indices than
  // rest
  template <typename... TArgs>
  general_tensor_t(data_t first_elem, TArgs... elem)
      : m_data({first_elem, static_cast<data_t>(elem)...}) {
    static_assert(sizeof...(TArgs) == ndof - 1,
                  "You need to specify exactly ndof arguments!");
    static_assert(utilities::all_true<(
                      std::is_convertible<data_t, TArgs>::value)...>::value,
                  "The data_types are incompatible!");
  };
  ...
  static inline __attribute__((always_inline)) constexpr size_t
  compressed_index() {
    ...
    return generic_symmetry_t<ndim, rank>::template compressed_index<
        a, indices...>::value;
  }

  //! Access the components of a tensor using (compile-time) natural indices
  //  This gives back a non-const reference, since this is a evaluated
  //  expression
  template <size_t... Ind> inline __attribute__((always_inline)) data_t &c() {
  ...
  template <size_t index> inline __attribute__((always_inline)) data_t &cc() {
  ...
  // CHECK: OBSOLETE but still used in expressions test, we should convert that
  // and remove this in that case we also need to remove it in expression base
  // class
  ...
  //! Access the components of a tensor using a (generic) compressed index
  inline __attribute__((always_inline)) T const &
  operator[](size_t const a) const {
    return m_data[a];
  }
  ...
  inline T & access(Ind_t... ind) {
    ...
    // Compute generic index (Fortran ordering)
    size_t indexL = 0;
    for (int i = 0; i < rank; ++i)
      indexL += indices[i] * static_cast<size_t>(std::pow(ndim, i));

    return access_guts(
        indexL,
        std::make_index_sequence<generic_symmetry_t<ndim, rank>::ndof>{});
  };
  ...
  template <size_t index>
  inline __attribute__((always_inline)) T const &evaluate() const {
    return m_data[symmetry_t::template index_from_generic<index>::value];
  }

  //! Set (part) of the tensor to the given expression
  //  Make sure to set a symmetric tensor to a symmetric tensor (cast), to
  //  remove redundant assignements
  ...
  inline __attribute__((always_inline)) void set(E const &e) {
    ...
    static_assert(sizeof...(Ind) == property_t::rank,
                  "You need to specify rank indices.");
    static_assert(num_free_indices == E::property_t::rank,
                  "You need to specify rank free indices.");
    static_assert(
        utilities::all_true<(
            (Ind >= -static_cast<int>(max_shift + 1)) &&
            (Ind < static_cast<int>(property_t::ndim)))...>::value,
        "Indices out of range. Use 0..ndim-1 to fix indices "
        "and -1...-(ndim - ndim_in)-1 to indicate a (shifted) free index.");
    static_assert(E::property_t::ndim <= property_t::ndim,
                  "Can't set a lower dim tensor to a higher dim.");
  ...
  //  This is not optimally optimized, please change if used in non-debugging
  //  environment Any expression passed as argument is converted to
  //  this_tensor_t automagically (if possible)
  template <int exponent>
  inline __attribute__((always_inline)) decltype(auto)
  compare_components(this_tensor_t const &t) {
  ...
  //  since they are implicitly convertible (this_tensor_t constructor from an
  //  expression).
  friend std::ostream &operator<<(std::ostream &stream,
                                  const this_tensor_t &t) {
    stream << "[";
  ...
  operator+=(tensor_expression_t<E> const &tensor_expression) {
    //    static_assert(std::is_same<frame_t, typename
    //    E::property_t::frame_t>::value,
    //                  "Frame types don't match!");
    ...
    add_to_tensor_t<E, this_tensor_t, ndof - 1>::add_to_tensor(
        tensor_expression, *this);
  };
  ...
  operator-=(tensor_expression_t<E> const &tensor_expression) {
    ...
    subtract_from_tensor_t<E, this_tensor_t, ndof - 1>::subtract_from_tensor(
        tensor_expression, *this);
  };
  ...
  inline __attribute__((always_inline)) void operator*=(data_t const &lambda) {
    ...
    multiply_tensor_with_t<this_tensor_t, ndof - 1>::multiply_tensor_with(
        lambda, *this);
  };
