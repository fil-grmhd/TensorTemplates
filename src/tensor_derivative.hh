...
public:
  ...
  static_assert( std::is_same<typename std::tuple_element<
                               0, typename beta_t::property_t::index_t>::type,
                           lower_t>::value,
  ...
  tensor_advective_derivative_t(size_t const grid_index_, array_t const & ptr_array_, beta_t const& beta_,
     			        fd_u_t const & fdu_, fd_d_t const & fdd_)
                            : grid_index(grid_index_), ptr_array(ptr_array_), beta(beta_) ,fdu(fdu_),
			      fdd(fdd_) {};
  ...
  template<size_t tensor_sym_index, size_t index, typename T, typename fdu_t, typename fdd_t, typename arrayT>
  struct beta_dE{
    public:
      inline __attribute__ ((always_inline)) decltype(auto) value(T const & beta, fdu_t const & fdu, fdd_t const & fdd, 
	                                                          arrayT const & ptr_array, size_t const grid_index){
       return beta_dE<tensor_sym_index,index-1,T,fdu_t,fdd_t,arrayT>::value(beta,fdu,fdd, ptr_array, grid_index) 
  ...
  template<size_t tensor_sym_index, typename T, typename fdu_t, typename fdd_t, typename arrayT>
  struct beta_dE<tensor_sym_index,0,T,fdu_t,fdd_t, arrayT>{
    public:
      inline __attribute__ ((always_inline)) decltype(auto) value(T const & beta , fdu_t const & fdu, fdd_t const & fdd,
	                                                          arrayT const & ptr_array, size_t const grid_index){
